...
for p in range(datapoints):
    ...
    for depth in depths : 
        ...
        if n_training == n_training_list[0] :
            rank_DQFIM_list_1.append(rank_DQFIM)
        
        if n_training == n_training_list[-1] :
            rank_DQFIM_list_inf.append(rank_DQFIM)

        # print(np.array(rank_DQFIM_list).reshape(-1,))
...
#plot R_inf vs M
plt.plot(np.array(number_of_parameters_list), np.array(rank_DQFIM_list_inf).flatten(), color = 'b', label=r'$R_\infty$')
plt.plot(np.array(number_of_parameters_list), np.array(rank_DQFIM_list_1).flatten(), color= 'b', label=r'$R_1$', linestyle='--')
plt.ylabel(r'$R_L$')
plt.xlabel('M')
plt.title(r'$R_\infty$ vs M')
plt.legend(loc="upper right")   
plt.savefig('figure/Rinf_vs_M.png', dpi=300, bbox_inches='tight')
