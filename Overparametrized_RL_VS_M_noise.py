...
type_state=0#input states for training, 0: haar random, 1: product states 2: 1 particle states
...
for noise_ratio in noise_levels:
    ...
    rank_DQFIM_lists_inf[noise_ratio] = []
    rank_DQFIM_lists_1[noise_ratio] = []
    ...
    for p in range(datapoints):
        ...
        for depth in depths:
            ...
            if n_training == n_training_list[0]:
                number_of_parameters_list.append(n_circuit_parameters)
            ...
            training_states_target = inject_label_noise(training_states_target, noise_ratio)
            ...
            test_states_target = inject_label_noise(test_states_target, noise_ratio)
            ...
            if n_training == n_training_list[0]:
                rank_DQFIM_lists_1[noise_ratio].append(rank_DQFIM)
            if n_training == n_training_list[-1]:
                rank_DQFIM_lists_inf[noise_ratio].append(rank_DQFIM)

            print("rank of data Quantum Fisher information metric", rank_DQFIM)

# Plot R_inf vs M for different noise ratios
plt.figure(figsize=(10, 6))
colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k']  # Define colors for different noise ratios
for idx, noise_ratio in enumerate(noise_levels):
    color = colors[idx % len(colors)]
    plt.plot(
        np.array(number_of_parameters_list),
        np.array(rank_DQFIM_lists_inf[noise_ratio]).flatten(),
        color=color,
        label=fr'$R_\infty$ (Noise: {int(noise_ratio * 100)}%)'
    )
    plt.plot(
        np.array(number_of_parameters_list),
        np.array(rank_DQFIM_lists_1[noise_ratio]).flatten(),
        color=color,
        label=fr'$R_1$ (Noise: {int(noise_ratio * 100)}%)',
        linestyle='--'
    )

plt.ylabel(r'$R_L$')
plt.xlabel('M')
plt.title(r'$R_\infty$ vs M for Different Noise Ratios')
plt.legend(loc="upper right")
plt.savefig('figure/Rinf_vs_M_noise_ratios.png', dpi=300, bbox_inches='tight')
plt.show()
