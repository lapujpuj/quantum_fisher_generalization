...
# 80 parameters anought and  50 data


depths = [1, 2, 3, 4, 5, 6, 7, 8, 9]
n_training_list = [1, 2, 3, 4, 5, 7, 8, 9, 10,11, 12, 13, 14, 15, 20, 30, 40, 50]


# # Unitary test
# n_qubits=1 #3 ##number of qubits
# n_test=1 ##number test states sampled from distribution
# maxiter_opt= 1
# depth_input=1 ##additional depth added for preparing particle conserved states
# fix_depth=1 ##add unitary of fixed depth at end of circuit to make more random


print("Number training states : ",n_training_list)
print("Number of test states : ",n_test)
...
print("Number of parameters : ",[get_circuit_parameters(depth, n_qubits, model) for depth in depths])
...
rank_DQFIM_list = []
number_training_step_list = []

# noise_type = "amplitude"  # Choose the type of noise: 'amplitude', 'phase', or 'bit_flip'
# noise_levels = [0.0, 0.01, 0.05, 0.10, 0.20]
cost_train_final_lists = {depth: [] for depth in depths}
cost_test_final_lists = {depth: [] for depth in depths}
rank_DQFIM_lists = {depth: [] for depth in depths}
number_training_step_lists = {depth: [] for depth in depths}


for depth in depths:
    # print(f"=== Processing with noise ratio: {int(noise_ratio * 100)}% and of type: {str(noise_type)} ===")
    print(f"=== Processing with depth: {depth} ===")
    print(f"=== Processing with number of parameters : {get_circuit_parameters(depth, n_qubits, model)} ===")
    ...
    for p in range(datapoints):
        ...
        number_training_step_list.append(n_iterations)
    ...
    rank_DQFIM_lists[depth] = rank_DQFIM_list
    cost_train_final_lists[depth] = cost_train_final_list
    cost_test_final_lists[depth] = cost_test_final_list
    number_training_step_lists[depth] = number_training_step_list
    print("=== Finished processing depth: {} ===".format(depth))

# Plot R_L vs number of parameters for overcomplete regime (fixed n_training = n_training_list[-1])
plt.figure(figsize=(10, 6))
fixed_n_training = n_training_list[-1]
rank_DQFIM_fixed_training = [rank_DQFIM_lists[depth][-1] for depth in depths]  # Take the last value for each depth

plt.plot([get_circuit_parameters(depth, n_qubits, model) for depth in depths], rank_DQFIM_fixed_training, marker='o')
plt.ylabel(r'$R_L$')
plt.xlabel("Number of parameters")
plt.title(f"Rank of data DQFIM vs number of parameters (L = {fixed_n_training})")
plt.grid(True)
plt.savefig("figure/Diagram/Rank_DQFIM_vs_parameters.png", dpi=300, bbox_inches='tight')
plt.show()

# Let's plot a phase diagram (in 2D for M and L) for the cost function with 2D meshgrid
parameters = np.array([get_circuit_parameters(depth, n_qubits, model) for depth in depths])
n_training_list = np.array(n_training_list)

cost_train_final_array = np.zeros((len(parameters), len(n_training_list)))  
cost_test_final_array = np.zeros((len(parameters), len(n_training_list)))
number_training_step_list_array = np.zeros((len(parameters), len(n_training_list)))
rank_DQFIM_array = np.zeros((len(parameters), len(n_training_list)))

# Transformation en grille 
parameters, n_training_list = np.meshgrid(parameters, n_training_list)


for i, depth in enumerate(depths):
    for j, n_training in enumerate(n_training_list[:, 0]):
        cost_train_final_array[i, j] = cost_train_final_lists[depths[i]][j]
        cost_test_final_array[i, j] = cost_test_final_lists[depths[i]][j]
        rank_DQFIM_array[i, j] = rank_DQFIM_lists[depths[i]][j]
        number_training_step_list_array[i, j] = number_training_step_lists[depths[i]][j]

print("n_training_list", n_training_list)
print("parameters", parameters)
print("cost_train_final_array_shape", cost_train_final_array.shape)
print("cost_train_final_array", cost_train_final_array)

# Plotting the phase diagram for cost function
plt.figure(figsize=(10, 6))
plt.pcolormesh(n_training_list, parameters, cost_train_final_array.T, cmap='viridis')
plt.colorbar(label='Cost Function')
plt.title('Phase Diagram of Training Cost')
plt.ylabel('Number of Parameters')
plt.xlabel('Number of Training States')
plt.grid(True)
plt.savefig("figure/Diagram/Phase_Diagram_Training_Cost.png", dpi=300, bbox_inches='tight')
plt.show()

# plotting the phase diagram for test cost function
plt.figure(figsize=(10, 6))
plt.pcolormesh( n_training_list,parameters , cost_test_final_array.T, cmap='viridis')
plt.colorbar(label='Cost Function')
plt.title('Phase Diagram of Test Cost')
plt.ylabel('Number of Parameters')
plt.xlabel('Number of Training States')
plt.grid(True)
plt.savefig("figure/Diagram/Phase_Diagram_Test_Cost.png", dpi=300, bbox_inches='tight')
plt.show()

# Plotting the phase diagram for rank of DQFIM
plt.figure(figsize=(10, 6))
plt.pcolormesh( n_training_list, parameters, rank_DQFIM_array.T, cmap='viridis')
plt.colorbar(label='Rank of DQFIM')
plt.title('Phase Diagram of Rank of DQFIM')
plt.ylabel('Number of Parameters')
plt.xlabel('Number of Training States')
plt.grid(True)
plt.savefig("figure/Diagram/Phase_Diagram_Rank_DQFIM.png", dpi=300) #, bbox_inches='tight')
plt.show()

plt.figure(figsize=(10, 6))
plt.pcolormesh(n_training_list, parameters, number_training_step_list_array.T, cmap='viridis')
plt.colorbar(label='Number of Training Steps')
plt.title('Phase Diagram of Number of Training Steps')
plt.ylabel('Number of Parameters')
plt.xlabel('Number of Training States')
plt.grid(True)
plt.savefig("figure/Diagram/Phase_Diagram_Number_Training_Steps.png", dpi=300, bbox_inches='tight')
plt.show()


# Save the results to a .mat file

scipy.io.savemat('data/phase_diagram_results.mat', {
    'depths': depths,
    'n_training_list': n_training_list,
    'cost_train_final_array': cost_train_final_array,
    'cost_test_final_array': cost_test_final_array,
    'rank_DQFIM_array': rank_DQFIM_array,
    'number_training_step_list_array': number_training_step_list_array,
})
